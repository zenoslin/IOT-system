...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SAMPLEAPP_SEND_PERIODIC_MSG_EVT )
  {
    ...
    Send_Message[0]='#';
    Send_Message[1]=wendu_shi+48;
    Send_Message[2]=wendu_ge+48;
    Send_Message[3]=shidu_shi+48;
    Send_Message[4]=shidu_ge+48;
    //数据发送函数，将采集到的信息发送给协调器
    if ( AF_DataRequest( &Point_To_Point_DstAddr,
                         &SampleApp_epDesc,
                         SAMPLEAPP_END1,
                         5,//osal_strlen(Send_Message),//发送数据个数
                         Send_Message,//发送数据指针
                         &SampleApp_TransID,
                         AF_DISCV_ROUTE,
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  {//协调器接收数据处理函数
    case SAMPLEAPP_END1:
     HalUARTWrite(0,&pkt->cmd.Data[0],18);//pkt->cmd.DataLength); //
     HalUARTWrite(0,"\n",1);  // 回车换行
     break;
